...
CREATE TABLE IF NOT EXISTS `concessionaria`.`cliente` (
  ...
  `endereço_cliente` VARCHAR(100) NULL,
...
CREATE TABLE IF NOT EXISTS `concessionaria`.`venda` (
  ...
  INDEX `fk_venda_cliente_idx` (`cliente_id_cliente` ASC, `cliente_cpf_cliente` ASC),
  INDEX `fk_venda_funcionario1_idx` (`funcionario_id_funcionario` ASC),
  INDEX `fk_venda_modelo1_idx` (`modelo_id_modelo` ASC),
  CONSTRAINT `fk_venda_cliente`
    FOREIGN KEY (`cliente_id_cliente` , `cliente_cpf_cliente`)
    REFERENCES `concessionaria`.`cliente` (`id_cliente` , `cpf_cliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
